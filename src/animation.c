...
#define ANGLE_TO_STEPS(target_angle)                                           \
    (((uint16_t) target_angle * NUM_STEPS_PER_ROT) / (uint16_t) 360)
...
static int _update_motor_anim(const struct anim_pos_t *anim_pos, uint8_t num_motors) {
    /* Basic config check */
    if (NULL == anim_pos) {
        return -1;
    }
    for (int motor_idx = 0; motor_idx < num_motors; motor_idx++) {
        if (anim_pos[motor_idx].motor_id >= NUM_MOTORS) {
    ...
    for (int motor_idx = 0; motor_idx < num_motors; motor_idx++) {
        const uint8_t motor_id = anim_pos[motor_idx].motor_id;
        _motors[motor_id].motor_status.anim_type =
                anim_pos[motor_idx].anim_type;
        _motors[motor_id].num_step_remaining =
                _process_num_steps(_motors[motor_id].current_step,
                                   anim_pos[motor_idx].position_degree,
                                   anim_pos[motor_idx].anim_angle_type);
...
static void _update_current_anim(void) {
    ...
    for (int motor_id = 0; motor_id < sequence->num_motors; motor_id++) {
        _update_motor_anim(sequence->position, sequence->num_motors);
    }
}

void process_anim_set(const struct anim_set_t *anim_set) {
